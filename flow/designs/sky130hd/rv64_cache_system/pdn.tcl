...
# sky130ram SRAM macro power pin (lowercase vdd)
...
# sky130ram SRAM macro ground pin (lowercase gnd)
add_global_connection -net {VSS} -inst_pattern {.*} -pin_pattern {^gnd$}
global_connect
####################################
# voltage domains
####################################
...
add_pdn_stripe -grid {grid} -layer {met1} -width {0.48} -pitch {10.88} -offset {0} -followpins
add_pdn_stripe -grid {grid} -layer {met4} -width {1.600} -pitch {54.280} -offset {13.570}
add_pdn_stripe -grid {grid} -layer {met5} -width {1.600} -pitch {54.400} -offset {13.600}
