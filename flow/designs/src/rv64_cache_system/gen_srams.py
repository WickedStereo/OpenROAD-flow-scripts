...
def gen_l2_arrays():
    ...
    s += "\toutput [16*64-1:0]  rdata_way_flat,\n"
    s += "\toutput [16*50-1:0]  tag_way_flat\n"
    s += ");\n\n"
    ...
    for w in range(16):
        ...
        s += f"\twire tag_we_{w} = tag_we & (way_sel == 4'd{w});\n"
        # Always read tags
        s += f"""\
...
# Here, we use port 1 for clean continuous reading of tag and data.
    ...
    for word in range(8):
        for m in range(8):
            ...
            s += f"\twire data_we_{word}_{m} = data_we & (word_sel == 3'd{word}) & ((way_sel == 4'd{way0}) | (way_sel == 4'd{way1}));\n"
            s += f"\twire [15:0] data_wmask_{word}_{m} = (way_sel == 4'd{way0}) ? {{8'h00, be}} : {{be, 8'h00}};\n"
            s += f"\twire [127:0] data_din_{word}_{m}  = (way_sel == 4'd{way0}) ? {{64'b0, wdata}} : {{wdata, 64'b0}};\n"
            # Read port 1
...
"""
    ...
    for w in range(16):
        s += f"\t\t(way_sel == 4'd{w}) ? rdata_way_{w} : \n"
    s += "\t\t64'b0;\n"

    s += "\n\tassign tag_selected = \n"
    for w in range(16):
        s += f"\t\t(way_sel == 4'd{w}) ? tag_way_flat[{w*50+49}:{w*50}] : \n"
    s += "\t\t50'b0;\n"
    s += "endmodule\n"
    return s
...
def gen_l2_directory():
    ...
    s += "\toutput wire [15:0]              rd_valid,\n"
    s += "\toutput wire [63:0]              rd_sharers,\n"
    s += "\toutput wire [15:0]              rd_owner_valid,\n"
    s += "\toutput wire [31:0]              rd_owner_id,\n"
    s += "\toutput wire [15:0]              rd_dirty,\n"
    s += "\tinput  wire                     we,\n"
    s += "\tinput  wire [7:0]               wr_set,\n"
    s += "\tinput  wire [3:0]               wr_way,\n"
