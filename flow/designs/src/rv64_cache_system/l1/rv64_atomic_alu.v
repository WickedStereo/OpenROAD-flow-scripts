// rv64_atomic_alu.v - Atomic Memory Operation ALU
...
module rv64_atomic_alu (
    input  [4:0]    amo_op,       // AMO operation type (funct5)
    input           amo_word,     // 1=word(.W), 0=doubleword(.D)
    input  [63:0]   old_value,    // Old value from memory
    input  [63:0]   operand,      // Operand value (from rs2)
    
    output reg [63:0] new_value   // New value to write to memory
);
    ...
    wire [31:0] old_word  = old_value[31:0];
    wire [31:0] op_word   = operand[31:0];
    ...
    wire signed [63:0] old_signed_dw = old_value;
    wire signed [63:0] op_signed_dw  = operand;
    
    // Comparison results (64-bit)
    wire old_less_signed_dw   = (old_signed_dw < op_signed_dw);
    wire old_less_unsigned_dw = (old_value < operand);
    ...
    always @(*) begin
        // Default: pass through old value
        result_dword = old_value;
        
        case (amo_op)
            AMO_SWAP: result_dword = operand;                                         // SWAP
            AMO_ADD:  result_dword = old_value + operand;                           // ADD
            AMO_XOR:  result_dword = old_value ^ operand;                           // XOR
            AMO_AND:  result_dword = old_value & operand;                           // AND
            AMO_OR:   result_dword = old_value | operand;                           // OR
            AMO_MIN:  result_dword = old_less_signed_dw   ? old_value : operand;    // MIN (signed)
            AMO_MAX:  result_dword = old_less_signed_dw   ? operand   : old_value;  // MAX (signed)
            AMO_MINU: result_dword = old_less_unsigned_dw ? old_value : operand;    // MINU (unsigned)
            AMO_MAXU: result_dword = old_less_unsigned_dw ? operand   : old_value;  // MAXU (unsigned)
            default:  result_dword = old_value;                                       // Default: no change
        endcase
    end
    ...
    always @(*) begin
        if (amo_word) begin
            ...
            new_value = { {32{result_word[31]}}, result_word };
        end else begin
            // Doubleword operation: use full 64-bit result
            new_value = result_dword;
        end
    end
